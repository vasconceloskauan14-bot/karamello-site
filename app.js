...
// ===== CPF / Idade (demo) =====
    ...
    function showScreen(which, instant=false){
      [screenStart, screenProcurar, screenProcurarEditor, screenClientProfile, screenClient, screenPickEditor, screenEditor, screenAuth].filter(Boolean).forEach(s => s.classList.remove('show'));
...
/* ===== Karameloo Beta Fix: Start entry + hide left nav + failsafe ===== */
(function(){
  ...
  // body class to hide nav on start screen
  function setAtStart(on){
    try{
      if(on) body.classList.add('atStart');
      else body.classList.remove('atStart');
    }catch(e){}
  }

  // expose simple entry helpers
  window.enterAsClient = function(){
    try{ setAtStart(false); }catch(e){}
    try{ safeHideLoading(); }catch(e){}
    try{
      role = 'client';
      if(typeof showScreen === 'function' && typeof screenClientProfile !== 'undefined' && screenClientProfile){
        showScreen(screenClientProfile, true);
        return;
      }
    }catch(e){}
    // fallback
    try{ if(typeof showScreen === 'function' && typeof screenProcurar !== 'undefined' && screenProcurar) showScreen(screenProcurar, true); }catch(e){}
  };

  window.enterAsEditor = function(){
    try{ setAtStart(false); }catch(e){}
    try{ safeHideLoading(); }catch(e){}
    try{
      role = 'editor';
      if(typeof showScreen === 'function' && typeof screenEditor !== 'undefined' && screenEditor){
        showScreen(screenEditor, true);
        return;
      }
    }catch(e){}
    try{ if(typeof showScreen === 'function' && typeof screenProcurarEditor !== 'undefined' && screenProcurarEditor) showScreen(screenProcurarEditor, true); }catch(e){}
  };
  ...
  function boot(){
    ...
    safeHideLoading();
    try{
      if(typeof showScreen === 'function' && typeof screenStart !== 'undefined' && screenStart){
        showScreen(screenStart, true);
      }
    }catch(e){}
  }
